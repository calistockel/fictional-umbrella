    ...
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[11.5px] text-ink-400">{commune.region}</p>
        </div>
        <div className="text-right">
          <div className="text-[20px] font-semibold tabular text-ink-900">{commune.developmentFriendliness}</div>
          <div className="text-[10.5px] text-ink-400">friendliness /10</div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2.5 border-t border-ink-100 pt-3.5">
        <Metric label="Approval rate" value={`${commune.approvalRate}%`} />
        <Metric label="Median decision" value={`${commune.medianDecisionDays}d`} />
        <Metric label="Incomplete files" value={`${commune.incompleteFileRate}%`} />
      ...
      <div className="mt-3.5 flex items-center justify-between">
        <span className={`flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-medium ${activityClasses[commune.permitActivity]}`}>
          {commune.permitActivity} activity
        </span>
        <span className="flex items-center gap-1 text-[11.5px] font-medium text-ink-500">
          <TrendIcon size={13} />
          density {commune.densityTrend === "flat" ? "stable" : commune.densityTrend}
        </span>
      </div>
